...
def prepare_token_dataset(filename: Path | str, seq_length: int = 100):
    ...
    return dataset, tokens, tok_to_int, n_token_vocab
...
def train_token_model(
    ...
    dataset, _, _, n_token_vocab = prepare_token_dataset(text_path, seq_length)
    ...
    model = TokenLSTM(n_token_vocab, hidden=hidden, dropout=dropout).to(device)
    ...
    for epoch in range(1, epochs + 1):
        model.train()
        bar = tqdm(loader, desc=f"Token epoch {epoch:02d}/{epochs}", unit="batch")
        for xb, yb in bar:
    ...
    checkpoint = output_dir / f"token-model-{epochs:02d}-epochs-{avg_loss:.4f}.pt"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train the token-level LSTM model.")
    parser.add_argument("text_path", nargs="?", default="data/text.txt")
    parser.add_argument("--epochs", type=int, default=1)
    parser.add_argument("--output-dir", default=".")
    args = parser.parse_args()

    ckpt, loss = train_token_model(
        args.text_path,
        epochs=args.epochs,
